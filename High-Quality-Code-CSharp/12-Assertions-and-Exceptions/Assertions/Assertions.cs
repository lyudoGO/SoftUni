...
class Assertions
{
    ...
    {
        // Check if the arguments are valid in Debbug mode
        Debug.Assert((startIndex >= 0) && (endIndex >= 0), "Start and end index of array cannot be negative.");
        Debug.Assert(startIndex <= endIndex, "Start index cannot be bigger than end index.");
        Debug.Assert(arr.Length >= endIndex, "End index cannot be bigger to array length.");
